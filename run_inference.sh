...
TREECOUNT=8
REPLICA_COUNT=2  # a number from 1 to 4
...
TARGETTREES=./QuartetMethods/example/trees.txt
...
for f in ${FACTORS[@]}; do
    echo "RUNNING "$f
    DATASET=./QuartetMethods/example/simulated_data-$f
    for setting in ${SETTINGS[@]}; do
        if $USE_MORPH; then 
            TREEOUTPUT=./QuartetMethods/outputs_big/outputs_with_morph/inference_outputs-$f/$setting'_noborrowing'
        else
            TREEOUTPUT=./QuartetMethods/outputs_big/outputs_no_morph/inference_outputs-$f/$setting'_noborrowing'
        fi
                    ...
                    if [[ $FILE =~ $pattern ]]; then
                        ...
                        if $DO_ASTRAL; then # run ASTRAL 
                            if ! grep -q $FILE $ASTRAL_SCOREOUTPUT; then
                                ...
                                java -jar ./QuartetMethods/ASTRAL/astral.5.7.8.jar -i scratch/tmp_quartet_$RUNID.txt -o $TREEOUTPUT/$ASTRAL_VARIANT/trees/$id.tre -e scratch/tmp_bipartitions_$RUNID.bootstrap.trees > /dev/null 2> $TREEOUTPUT/$ASTRAL_VARIANT/logs/$id.log # Run ASTRAL in default mode
                                echo "✅ Heuristic ASTRAL tree inference" 

                                echo $FILE >> $ASTRAL_SCOREOUTPUT
                                if [ $QT_MODE != 4 ]; then 
                                    Rscript ./QuartetMethods/scripts/QuartetScorer.R -f newick -r $CURRENT_TREE -m 1 -p 0 -i $TREEOUTPUT/$ASTRAL_VARIANT/trees/$id.tre >> $ASTRAL_SCOREOUTPUT
                                else 
